...
# 讓程序支援 DPI 縮放，解決在 Windows 縮放（如 125%、150%）下截圖不全與點擊偏移問題
try:
    import ctypes
...
class WindowCaptureBot:
    def __init__(self):
        ...
        # 偵測 Windows 系統 DPI 縮放比例
        try:
            import ctypes
            # GetDpiForSystem 為 Windows 10 1607+ 首選 API，不受進程 DPI Aware 模式影響
            try:
        ...
        self.friendship_target = None
        
        # ✅ 新增：當前資源變數
        self.current_gold = 0
        self.current_skystone = 0
    ...
    def update_ui_texts(self):
        ...
        self.friendship_target_label.config(text="友情書籤")
    ...
    def setup_auto_count_ui(self, parent_frame):
        ...
        # ✅ 第三行：友情書籤目標
        row3_frame = ttk.Frame(parent_frame)
        row3_frame.grid(row=2, column=0, columnspan=4, sticky=(tk.W, tk.E), pady=(5, 0))
        
        self.friendship_target_label = ttk.Label(row3_frame, text="友情書籤", font=("Arial", 10))
        self.friendship_target_label.grid(row=0, column=0, padx=(0, 5))
        
        self.friendship_target_var = tk.StringVar()
        friendship_target_entry = ttk.Entry(row3_frame, textvariable=self.friendship_target_var, width=10)
        friendship_target_entry.grid(row=0, column=1, padx=(0, 20))
        self.ui_controls.append(friendship_target_entry)
        
        # ✅ 重置所有目標按鈕
        self.reset_targets_btn = ttk.Button(row3_frame, text=self.get_text("reset_all_targets"), command=self.reset_targets)
        self.reset_targets_btn.grid(row=0, column=2, padx=(20, 0))
        self.ui_controls.append(self.reset_targets_btn)
        
        # ✅ 第四行：當前資源設定
        row4_frame = ttk.Frame(parent_frame)
        row4_frame.grid(row=3, column=0, columnspan=4, sticky=(tk.W, tk.E), pady=(10, 0))
        
        gold_label = ttk.Label(row4_frame, text="當前金幣", font=("Arial", 10))
        gold_label.grid(row=0, column=0, padx=(0, 5))
        
        self.current_gold_var = tk.StringVar()
        gold_entry = ttk.Entry(row4_frame, textvariable=self.current_gold_var, width=12)
        gold_entry.grid(row=0, column=1, padx=(0, 20))
        self.ui_controls.append(gold_entry)
        
        skystone_label = ttk.Label(row4_frame, text="當前天空石", font=("Arial", 10))
        skystone_label.grid(row=0, column=2, padx=(0, 5))
        
        self.current_skystone_var = tk.StringVar()
        skystone_entry = ttk.Entry(row4_frame, textvariable=self.current_skystone_var, width=12)
        skystone_entry.grid(row=0, column=3, padx=(0, 20))
        self.ui_controls.append(skystone_entry)
    ...
    def reset_targets(self):
        ...
        self.friendship_target_var.set("")
        self.current_gold_var.set("")
        self.current_skystone_var.set("")
    ...
    def capture_window(self, hwnd):
        ...
        try:
            ...
            # 如果解析度不是 640x360，強制縮小至 640x360 以適應原本的模板匹配
            if gimg.shape[1] != 640 or gimg.shape[0] != 360:
                gimg = cv2.resize(gimg, (640, 360), interpolation=cv2.INTER_AREA)
    ...
    def click_at_position(self, hwnd, x, y, click_time = 3):
        ...
        try:
            real_x = int(x * self.dpi_scale)
            real_y = int(y * self.dpi_scale)
            lParam_start = win32api.MAKELONG(real_x, real_y)
    ...
    def simulate_vertical_scroll(self, hwnd, start_x, start_y, distance=100):
        """更真實的長按拖動模擬"""
        try:
            real_start_x = int(start_x * self.dpi_scale)
            real_start_y = int(start_y * self.dpi_scale)
            real_distance = int(distance * self.dpi_scale)
            
            # 1. 發送鼠標移動到起始位置
            lParam_start = win32api.MAKELONG(real_start_x, real_start_y)
            win32gui.PostMessage(hwnd, win32con.WM_MOUSEMOVE, 0, lParam_start)
            time.sleep(0.05)
            ...
            # 3. 平滑拖動
            end_y = real_start_y - real_distance
            current_y = real_start_y
            step_size = int(20 * self.dpi_scale)
            if step_size < 1:
                step_size = 1
    ...
    def check_resources_available(self, selected_names):
        """✅ 檢查是否有足夠的資源來購買書籤"""
        try:
            gold_text = self.current_gold_var.get().strip()
            skystone_text = self.current_skystone_var.get().strip()
            
            # 如果沒有設定資源限制，直接返回可用
            if not gold_text and not skystone_text:
                return True, "資源未設定，無限制"
            
            current_gold = int(gold_text) if gold_text else float('inf')
            current_skystone = int(skystone_text) if skystone_text else float('inf')
            
            # 確定需要的最大金幣數（根據選中的書籤）
            required_gold = 0
            for name in selected_names:
                if name == "covenant.png":
                    required_gold = max(required_gold, 184000)
                elif name == "mystic.png":
                    required_gold = max(required_gold, 280000)
                elif name == "friend.png":
                    required_gold = max(required_gold, 18000)
            
            # 每次刷新消耗3個天空石
            required_skystone = 3
            
            # 檢查金幣
            if gold_text and current_gold < required_gold:
                return False, f"金幣不足 (需要:{required_gold}, 當前:{current_gold})"
            
            # 檢查天空石
            if skystone_text and current_skystone < required_skystone:
                return False, f"天空石不足 (需要:{required_skystone}, 當前:{current_skystone})"
            
            return True, "資源充足"
            
        except ValueError:
            return True, "資源設定無效，視為無限制"
        except Exception as e:
            self.log_message(f"資源檢查錯誤: {e}")
            return True, "資源檢查出錯"
    
    def deduct_resources(self, item_name):
        """✅ 扣除購買消耗的金幣和天空石"""
        try:
            gold_text = self.current_gold_var.get().strip()
            skystone_text = self.current_skystone_var.get().strip()
            ...
            current_gold = int(gold_text) if gold_text else 0
            current_skystone = int(skystone_text) if skystone_text else 0
            
            consumed_gold = 0
            if item_name == "covenant.png":
                consumed_gold = 184000
            elif item_name == "mystic.png":
                consumed_gold = 280000
            elif item_name == "friend.png":
                consumed_gold = 18000
            
            # 扣除資源
            if gold_text and consumed_gold > 0:
                current_gold -= consumed_gold
                self.current_gold_var.set(str(max(current_gold, 0)))
            
            if skystone_text:
                current_skystone -= 3
                self.current_skystone_var.set(str(max(current_skystone, 0)))
                
        except Exception as e:
            self.log_message(f"資源扣除錯誤: {e}")

    def update_current_resources(self, gold_delta=0, skystone_delta=0):
        """更新當前金幣和天空石顯示"""
        try:
            if gold_delta != 0:
                gold_text = self.current_gold_var.get().strip()
            ...
            if skystone_delta != 0:
                skystone_text = self.current_skystone_var.get().strip()
                current_skystone = int(skystone_text) if skystone_text else 0
                current_skystone += skystone_delta
                self.current_skystone_var.set(str(max(current_skystone, 0)))
        except ValueError:
            pass
        except Exception as e:
            self.log_message(f"更新資源顯示錯誤: {e}")
    ...
    def resize_target_window(self, hwnd):
        ...
        try:
            ...
            target_w = int(640 * self.dpi_scale)
            target_h = int(360 * self.dpi_scale)
            ...
            try:
                ...
                for template, name in zip(selected_templates, selected_names):
                    ...
                    if match_loc is not None:
                        ...
                        if is_clickable:
                            # ✅ 檢查資源是否充足
                            has_resources, resource_msg = self.check_resources_available(selected_names)
                            if not has_resources:
                                self.log_message(f"停止刷取: {resource_msg}", color="red")
                                self.stop_capture()
                                break
                            ...
                            if name == "covenant.png":
                                ...
                                # 計算進度顯示（次數 = 數量 / 5）
                                covenant_count = self.stats['covenant_bookmarks'] // 5
                                try:
                                    ...
                                    if covenant_target_text and covenant_target_text != "0":
                                        ...
                                        progress_msg = f"找到聖約書籤 ({covenant_count}/{covenant_target})"
                                    else:
                                        progress_msg = f"找到聖約書籤 ({covenant_count})"
                                except:
                                    progress_msg = f"找到聖約書籤 ({covenant_count})"
                                self.log_message(progress_msg, color="blue")
                                # ✅ 扣除資源
                                self.deduct_resources(name)
                            elif name == "mystic.png":
                                self.stats['mystic_bookmarks'] += 50
                                self.stats['gold_consumed'] += 280000
                                # 計算進度顯示（次數 = 數量 / 50）
                                mystic_count = self.stats['mystic_bookmarks'] // 50
                                try:
                                    ...
                                    if mystic_target_text and mystic_target_text != "0":
                                        ...
                                        progress_msg = f"找到神秘書籤 ({mystic_count}/{mystic_target})"
                                    else:
                                        progress_msg = f"找到神秘書籤 ({mystic_count})"
                                except:
                                    progress_msg = f"找到神秘書籤 ({mystic_count})"
                                self.log_message(progress_msg, color="red")
                                # ✅ 扣除資源
                                self.deduct_resources(name)
                            elif name == "friend.png":
                                ...
                                # 計算進度顯示（次數 = 數量 / 5）
                                friendship_count = self.stats['friendship_bookmarks'] // 5
                                try:
                                    friendship_target_text = self.friendship_target_var.get().strip()
                                    if friendship_target_text and friendship_target_text != "0":
                                        friendship_target = int(friendship_target_text)
                                        progress_msg = f"找到友情書籤 ({friendship_count}/{friendship_target})"
                                    else:
                                        progress_msg = f"找到友情書籤 ({friendship_count})"
                                except:
                                    progress_msg = f"找到友情書籤 ({friendship_count})"
                                self.log_message(progress_msg, color="green")
                                # ✅ 扣除資源
                                self.deduct_resources(name)
                ...
                try:
                    ...
                    # 檢查聖約書簽目標（以每次 5 張書籤為單位）
                    if covenant_target_text and covenant_target_text != "0":
                        covenant_target = int(covenant_target_text)
                        covenant_count = self.stats['covenant_bookmarks'] // 5
                        if covenant_count >= covenant_target:
                            target_reached = True
                            target_messages.append(f"聖約書簽達標 ({covenant_count}/{covenant_target})")
                    
                    # 檢查神秘書簽目標（以每次 50 張書籤為單位）
                    if mystic_target_text and mystic_target_text != "0":
                        mystic_target = int(mystic_target_text)
                        mystic_count = self.stats['mystic_bookmarks'] // 50
                        if mystic_count >= mystic_target:
                            target_reached = True
                            target_messages.append(f"神秘書簽達標 ({mystic_count}/{mystic_target})")
                    
                    # 檢查友情書簽目標（以每次 5 張書籤為單位）
                    if friendship_target_text and friendship_target_text != "0":
                        friendship_target = int(friendship_target_text)
                        friendship_count = self.stats['friendship_bookmarks'] // 5
                        if friendship_count >= friendship_target:
                            target_reached = True
                            target_messages.append(f"友情書簽達標 ({friendship_count}/{friendship_target})")
                ...
                if self.is_running:
                    ...
                    self.update_current_resources(skystone_delta=-3)
                    self.update_statistics_display()
                    self.update_auto_count_display()
    ...
    def check_clickable_status(self, image, match_x, match_y):
        ...
        try:
            ...
            # NOTE: 為了在 640x360 解析度下容許畫面微小位移，將 ROI 檢測框往左上方擴充並加大寬高，
            # 讓 cv2.matchTemplate 有更大的滑動搜尋空間 (寬度由30增至50，高度由20增至30)
            roi_x_start = check_x - 10
            roi_y_start = check_y - 5
            roi_x_end = check_x + 40
            roi_y_end = check_y + 25
            
            if roi_x_start < 0 or roi_y_start < 0 or roi_x_end > w or roi_y_end > h:
                return False, self.get_text("log_region_out_of_range")
            
            # 提取ROI區域
            roi_gray = image[roi_y_start:roi_y_end, roi_x_start:roi_x_end]
            ...
            if template_11 is not None and template_01 is not None:
                ...
                # NOTE: 調降閾值至 0.55，避免在低解析度或抗鋸齒影響下，正常文字之匹配率略低於原本的 0.7 而被誤判為已購買
                threshold = 0.55
    ...
    def draw_debug_rectangle(self, image, match_x, match_y, is_clickable):
        ...
        try:
            ...
            roi_x_start = max(0, check_x - 10)
            roi_y_start = max(0, check_y - 5)
            roi_x_end = min(w, check_x + 40)
            roi_y_end = min(h, check_y + 25)
            
            if check_x < 0 or check_y < 0 or check_x + 25 > w or check_y + 13 > h:
            ...
            # NOTE: 在 debug 圖中同時畫出黃色框(代表擴大後實際檢測的 ROI 範圍)與對應狀態的顏色框(代表理論文字框)
            # 畫擴大後的判定區域 ROI (線寬2，黃色以示區別)
            cv2.rectangle(image, (roi_x_start, roi_y_start), (roi_x_end, roi_y_end), (0, 255, 255), 2)
            
            # 畫原有的理論文字框 (線寬2，狀態顏色)
            cv2.rectangle(image, (check_x, check_y), (check_x + 25, check_y + 13), color, 2)
            
            # 在框下方加文字標示 (微調位置至 ROI 底部外側)
            text_y = roi_y_end + 15 if roi_y_end + 15 < h else roi_y_start - 5
    ...
    def save_settings(self):
        ...
        settings = {
            ...
            "friendship_target": self.friendship_target_var.get(),
            "current_gold": self.current_gold_var.get(),
            "current_skystone": self.current_skystone_var.get(),
    ...
    def load_settings(self):
        ...
        try:
            if os.path.exists("settings.json"):
                ...
                self.friendship_target_var.set(settings.get("friendship_target", ""))
                self.current_gold_var.set(settings.get("current_gold", ""))
                self.current_skystone_var.set(settings.get("current_skystone", ""))
    ...
    def run(self):
        ...
        self.log_message(f"[DPI] 偵測到系統縮放比例為: {self.dpi_scale * 100:.0f}% (scale={self.dpi_scale})", color="blue")
        self.root.mainloop()
